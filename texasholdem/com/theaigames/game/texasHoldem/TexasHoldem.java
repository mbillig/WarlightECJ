...
public class TexasHoldem implements Logic
{
	...
	public void finish() throws Exception
	{
        ...
        gameDone = true;
        if (this.matchPlayer.getWinner() == null)
            winner = 0;
        else if (this.matchPlayer.getWinner().getName() == "player1")
            winner = 1;
        else
            winner = 2;

        score = this.matchPlayer.getHandNumber();
	}
	...
	{	
        ...
        while (running) {
            if (gameDone) {
                ...
                running = false;
                TexasResults TR = TexasResults.getInstance();
                TR.setScore(score);
                TR.setWinner(winner);
            }
        }
